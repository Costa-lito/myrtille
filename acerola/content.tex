...
Acerola est une charte graphique pensée pour produire des documents techniques avec un rendu clair et moderne. Son utilisation est faite pour être simple grâce à l'utilisation d'une classe \LaTeX{} standard. Acerola est dérivée de la classe Myrille dont elle reprend les éléments et mises en page mais change totalement la palette de couleurs pour s'orienter vers des teintes rouge ruby.

% ── SECTION 1 : COULEURS ───────────────────────────────────────
\section{Palette de Couleurs}

Cette section détaille l'ensemble des couleurs définies dans la classe de Acerola. Elles sont organisées par thématique pour faciliter leur utilisation. 
...
La classe Acerola utilise par défaut un système de notes de bas de page symboliques\footnote{Ceci est un exemple de note de bas de page symbolique.} plutôt que numériques. Cela permet d'éviter toute confusion avec des appels de bibliographie ou des exposants mathématiques, tout en apportant une touche d'élégance éditoriale.
...
La classe Acerola intègre des boîtes spécifiques pour mettre en valeur les théorèmes, propriétés et définitions. Ces environnements sont numérotés automatiquement par section et supportent les références croisées.
...
La classe Acerola propose plusieurs environnements pour intégrer des images de manière élégante et cohérente avec la charte graphique. Chaque encadré est conçu pour un usage précis : mise en valeur, comparaison texte-image, note latérale ou figure numérotée.
...
\begin{imgbox}[Acerola — Malpighia emarginata]{12cm}{acerola.jpg}
  \footnotesize\color{mid}\textit{L'acérola, fruit emblématique qui donne son nom à cette charte graphique.}
\end{imgbox}

\textbf{Code source :}
\begin{texblock}
\begin{imgbox}[Légende du titre]{12cm}{chemin/vers/image.jpg} % Laisser la taille vide pour auto
...
\begin{imgfloat}{acerola.jpg}
  \textbf{Malpighia emarginata}

  L'acérola (ou cerise des Antilles) est un petit fruit rouge vif, juteux et acidulé, exceptionnellement riche en vitamine C. Il pousse naturellement dans les régions tropicales d'Amérique du Sud et des Caraïbes.

  \vspace{4pt}
  Son nom a été choisi pour cette charte car il évoque le dynamisme, la fraîcheur et l'élégance des teintes rouge cerise et bordeaux utilisées dans la palette de couleurs Acerola.
\end{imgfloat}
...
\imgsidenote{acerola.jpg}{Cette miniature de l'acérola illustre l'utilisation de la commande \texttt{\textbackslash imgsidenote}, idéale pour associer une petite image à une note informative sans perturber le flux du texte.}
...
\myrfig[width=0.5\linewidth]{acerola.jpg}{L'acérola (\textit{Malpighia emarginata}), fruit éponyme de la charte graphique.}
...
La classe Acerola intègre un générateur automatique de page de garde. La personnalisation s'effectue de manière claire et centralisée dans le préambule de votre document (avant le \ci{\textbackslash begin\{document\}}), à l'aide de macros dédiées.
...
\addcontentsline{toc}{section}{Licences et Crédits}

\begin{description}
    \item[Auteur :] Ce projet et ce document ont été créés par \textbf{Costa Zaffani-Le Carpentier}.
    \item[Code source :] Code source \texttt{.tex} et classes \texttt{.cls} sous licence \textbf{MIT}.
    \item[Image \textit{acerola.jpg} :] Par \textbf{Marcelo P. B. Silva} (\href{https://commons.wikimedia.org/wiki/File:Acerola_Malpighia_glabra.jpg}{Source}) sous licence \href{https://creativecommons.org/publicdomain/zero/1.0/deed.fr}{CC0 - Public Domain}.
