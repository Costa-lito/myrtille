...
\documentclass[theme=myrtille]{myrtille}
...
\coverfeatures{
  \begin{tabular}{@{}ll@{}}
    \color{coverMuted}\sffamily\small Identité visuelle & \color{accent}\sffamily\small \textbullet\ Palette de couleurs, typographie \\[3pt]
...
\headerleft{Myrtille \textbf{·} Charte Graphique}
...
Bien que ce document présente la palette bleue par défaut (\ci{theme=myrtille}), la classe embarque nativement deux autres thèmes chromatiques complets que vous pouvez activer lors de l'appel de la classe :
\begin{itemize}
...
Cette section détaille l'ensemble des couleurs définies dans la classe de Myrtille (thème bleu). Elles sont organisées par thématique pour faciliter leur utilisation.
...
\begin{warnbox}
Ceci est une boîte d'avertissement générée avec \ci{\textbackslash begin\{warnbox\}}. Elle utilise des nuances de jaune/brun pour capter l'attention de l'utilisateur sur une action potentiellement destructrice ou une contrainte forte.
\end{warnbox}
...
La classe définit plusieurs environnements distincts, basés sur \ci{tcolorbox} et \ci{listings}, pour présenter du code source de manière élégante avec une barre latérale colorée.
...
\begin{texblock}
% Exemple pour ajouter du JavaScript avec une bordure verte (success) :
\newmyrtilleblock{jsblock}{JavaScript}{success}
...
La classe Myrtille propose plusieurs environnements pour intégrer des images de manière élégante et cohérente.

\subsection{Carte image (imgbox)}
\begin{imgbox}[La myrtille]{0.8\linewidth}{pictures/myrtille.jpg}
    Sous-arbrisseau produisant le fruit emblématique qui donne son nom à cette charte graphique.
\end{imgbox}

\begin{texblock}
\begin{imgbox}[Légende du titre]{0.8\linewidth}{pictures/myrtille.jpg}
Texte optionnel sous l'image.
\end{imgbox}
\end{texblock}

\subsection{Image et texte côte à côte (imgfloat)}
\begin{imgfloat}{pictures/myrtille.jpg}
\textbf{Vaccinium myrtillus}\\[4pt]
La myrtille commune est un petit fruit poussant sur un arbrisseau touffu. Elle est reconnaissable à sa couleur bleu violacé profond et sa chair légèrement acidulée.

Son nom a été choisi pour cette charte car il évoque la clarté, l'accessibilité et l'élégance sobre des teintes bleu marine et azur utilisées dans la palette de couleurs par défaut de la classe.
\end{imgfloat}

\subsection{Note latérale illustrée (\textbackslash imgsidenote)}
\imgsidenote{pictures/myrtille.jpg}{Cette miniature illustre l'utilisation de la commande \ci{\textbackslash imgsidenote}. Idéale pour associer une petite image à une note informative sans perturber le flux du texte.}
...
\myrfig[width=0.5\linewidth]{pictures/myrtille.jpg}{La myrtille commune (Vaccinium myrtillus), plante éponyme de la charte graphique.}
...
\begin{itemize}
    \item \textbf{Auteur :} Ce projet et ce document ont été créés par Costa Zaffani-Le Carpentier.
    \item \textbf{Code source :} Fichiers \ci{.tex} et classe \ci{myrtille.cls} sous licence MIT.
    \item \textbf{Image \textit{myrtille.jpg} :} Par \textbf{Ryan Hodnett} (\href{https://commons.wikimedia.org/wiki/File:Common_Bilberry_(Vaccinium_myrtillus)_-_Bergen,_Norway_2021-07-31_(03).jpg}{Source}) sous licence \href{https://creativecommons.org/licenses/by-sa/4.0/deed.fr}{CC-BY-SA 4.0}.
